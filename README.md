...
Frontend
React Native
Expo (com Expo Router v5)
TypeScript
Expo Location (coleta de posição do usuário)
react-native-maps + Google Maps
Google Maps Platform (geocodificação, rotas e visualização)
Axios (consumo da API)
Expo Dev Client
Backend
Node.js + Express (API REST atual do protótipo)
PostgreSQL + PostGIS (armazenamento e consultas espaciais)
Sequelize ORM (modelagem e acesso ao banco)
@turf/turf (operações geoespaciais auxiliares no backend)
...
npx expo start -c
➡️ Emulador Android 1. Configure o Android Studio. 2. Crie e inicie um dispositivo virtual (AVD). 3. Rode:

npx expo run:android
➡️ Simulador iOS (somente macOS) 1. Instale o Xcode. 2. Rode:
